...
for i_episode in range(num_episodes):
    ...
    l_a = 0
    for t in count():
        action = int(torch.tensor([[env.action_space.sample()]], dtype=torch.long))
...
# np.savetxt('results_beam_tracking/reward_episode_random', reward_episode)
# np.savetxt('results_beam_tracking/mean_packet_rate_episode_random', mean_packet_rate_episode)
# np.savetxt('results_beam_tracking/action_list_random', action_list)
# np.savetxt('results_beam_tracking/failure_type_random', info['failure_type'])
# # np.savetxt('results_beam_tracking/total_traffic_cttc', info['total_traffic'])
# np.savetxt('results_beam_tracking/th_random', th_episode)
